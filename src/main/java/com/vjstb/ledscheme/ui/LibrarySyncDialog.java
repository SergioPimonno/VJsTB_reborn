 ...
 *
 * <p><b>Не единственный путь синхронизации</b> — {@code App.syncLibraryInBackground}
 * делает ТО ЖЕ САМОЕ (та же пара {@code fetchChanges}/{@code applyLibrarySyncItems})
 * автоматически при КАЖДОМ запуске приложения, тихо и без диалога (баг-репорт:
 * "при запуске приложения как будто не происходит автоматической синхронизации с
 * сервером, только по нажатии кнопки" — раньше это было ровно так, автозапуска не
 * было вовсе). Этот диалог остаётся для СЛУЧАЕВ, когда пользователь хочет явное
 * подтверждение результата (прогресс-бар, итоговое сообщение) или явную ошибку,
 * если сервер недоступен — тихий автозапуск при старте такой обратной связи
 * сознательно не даёт.
 */
